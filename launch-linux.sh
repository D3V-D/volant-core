...
if ! command -v uv >/dev/null 2>&1; then
    ...
    echo "    Re-run volant-install-linux.sh from the parent folder to set up dependencies."
...
open_browser() {
    if command -v xdg-open >/dev/null 2>&1; then
        xdg-open "$VOLANT_URL" >/dev/null 2>&1
    elif command -v gio >/dev/null 2>&1; then
        gio open "$VOLANT_URL" >/dev/null 2>&1
    else
        echo "[!] Could not find xdg-open or gio. Open $VOLANT_URL in your browser."
    fi
}

minimize_terminal() {
    if [[ -n "${WINDOWID:-}" ]] && command -v xdotool >/dev/null 2>&1; then
        xdotool windowminimize "$WINDOWID" >/dev/null 2>&1 || true
    fi
}

echo "[+] Starting Volant on $VOLANT_URL ..."
echo "    Your browser will open when the app is ready."
echo "    This window will minimize a few seconds after that when supported by your desktop environment."
